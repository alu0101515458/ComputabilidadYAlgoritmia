...
// MODIFICACION
void EliminarNota(std::multimap<std::string, float>& smap) {
  float nota;
  std::string afirmacion;
  std::cout << "Desea eliminar alguna nota?: ";
  std::cin >> afirmacion;
  if(afirmacion == "si") {
    while(afirmacion != "no") {
      if(afirmacion == "si") {
        std::cout << "Introduce nota a eliminar: ";
        std::cin >> nota;
        for(const auto& p: smap) if(p.second == nota) smap.erase(p.first);
        std::cout << "\n";
        std::cout << "Nuevas Notas" << std::endl;
        std::string clave = "";
        for(const auto& p: smap) {
      ...
      std::cout << "\n";
      std::cout << "Desea eliminar alguna nota mas?: ";
      std::cin >> afirmacion;
    }
  }
}
